...
class SessionManager:
    ...
    def _recover_assets_from_disk(self) -> bool:
        """
        Scan session subdirs and backfill empty asset fields in meta.
        Returns True (and saves) if anything was recovered.
        """
        changed = False

        # ── images ────────────────────────────────────────────────────────
        if not self.meta.get("image_paths"):
            pngs = sorted(
                (os.path.join(self.images_dir, f) for f in os.listdir(self.images_dir)
                 if f.lower().endswith(".png") and os.path.isfile(os.path.join(self.images_dir, f))),
                key=os.path.getmtime,
            )
            if pngs:
                self.meta["image_paths"] = pngs
                changed = True

        # ── audio ─────────────────────────────────────────────────────────
        if not self.meta.get("audio_path"):
            wavs = sorted(
                (os.path.join(self.audio_dir, f) for f in os.listdir(self.audio_dir)
                 if f.lower().endswith((".wav", ".mp3")) and os.path.isfile(os.path.join(self.audio_dir, f))),
                key=os.path.getmtime,
                reverse=True,
            )
            if wavs:
                self.meta["audio_path"] = wavs[0]
                changed = True
        ...
        if not self.meta.get("video_path"):
            mp4s = sorted(
                (os.path.join(self.video_dir, f) for f in os.listdir(self.video_dir)
                 if f.lower().endswith(".mp4") and os.path.isfile(os.path.join(self.video_dir, f))),
                key=os.path.getmtime,
                reverse=True,
            )
            if mp4s:
                self.meta["video_path"] = mp4s[0]
                changed = True
    ...
    def __init__(self, session_id: str, initial_name: str = ""):
        ...
        if os.path.exists(self.meta_path):
            ...
            if self._recover_assets_from_disk():
                pass  # _save_meta already called inside
            else:
                self._save_meta()
        else:
            self.meta = self._build_default_meta(session_id)
...
def _archives_dir() -> str:
    d = os.path.join(_mp_dir(), "archives")
    os.makedirs(d, exist_ok=True)
    return d


def delete_session(session_id: str) -> bool:
    """Move a session directory to .mp/archives/ instead of permanently deleting it.
    Returns True on success, False if not found."""
    s = get_session(session_id)
    if s is None:
        return False

    archives_root = _archives_dir()
    folder_name = os.path.basename(s.session_dir)

    # Ensure unique destination name in archives
    candidate = folder_name
    idx = 2
    while os.path.exists(os.path.join(archives_root, candidate)):
        candidate = f"{folder_name}-{idx}"
        idx += 1

    dest = os.path.join(archives_root, candidate)
    try:
        os.rename(s.session_dir, dest)
    except OSError:
        import shutil
        try:
            shutil.move(s.session_dir, dest)
        except OSError:
            return False
    return True
