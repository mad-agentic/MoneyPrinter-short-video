...
class DedupLoggingMiddleware(BaseHTTPMiddleware):
    """Middleware that deduplicates consecutive identical access logs"""
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Format log message
        log_msg = f'{request.client.host} - "{request.method} {request.url.path} HTTP/{request.scope["http_version"]}" {response.status_code}'
...
app.add_middleware(DedupLoggingMiddleware)
...
@app.delete("/system/sessions/{session_id}")
...
def patch_config(body: ConfigUpdateBody):
    ...
    # Keep runtime LLM selection in sync with persisted config.
    if any(k in incoming for k in ("llm_backend", "openai_model", "ollama_model")):
        try:
            from llm_provider import select_model

            backend = str(cfg.get("llm_backend", "ollama")).strip()
            if backend == "openai_compatible":
                selected = str(cfg.get("openai_model", "")).strip()
            else:
                selected = str(cfg.get("ollama_model", "")).strip()

            # Empty string intentionally clears old in-memory cache.
            select_model(selected)
        except Exception:
            # Saving config should succeed even if runtime model sync fails.
            pass
...
@app.get("/system/llm/models")
def get_llm_models():
    """List available models from the currently configured LLM backend."""
    try:
...
if __name__ == "__main__":
    ...
    # Run with custom middleware logging on port 15002 (avoiding port conflicts)
    uvicorn.run("api.main:app", host="127.0.0.1", port=15002, reload=False, access_log=False)
